...
import type { GradProgramKind, PartnerState } from "../game/types";
...
export function useGame() {
  ...
  const finishCompetition = useCallback(
    (activity: string, prep: PrepLevel, ratio: number) =>
      runAction((c) => completeCompetition(c, activity, prep, ratio)),
    [runAction],
  );
  const passCompetition = useCallback(
    (activity: string) => runAction((c) => skipCompetition(c, activity)),
    [runAction],
  );
  const datingAskOut = useCallback(
    (cand: PartnerState) => runAction((c) => askOut(c, cand)),
    [runAction],
  );
  const partnerTime = useCallback(() => runAction((c) => spendTimeWithPartner(c)), [runAction]);
  const partnerProgress = useCallback(() => runAction((c) => progressRelationship(c)), [runAction]);
  const partnerBreakUp = useCallback(() => runAction((c) => breakUp(c)), [runAction]);
  const tryBaby = useCallback(() => runAction((c) => haveBaby(c)), [runAction]);
  const childAction = useCallback(
    (kidId: string, action: ChildAction) => runAction((c) => interactChild(c, kidId, action)),
    [runAction],
  );
  const parentAction = useCallback(
    (relId: string, action: ParentAction) => runAction((c) => interactParent(c, relId, action)),
    [runAction],
  );
