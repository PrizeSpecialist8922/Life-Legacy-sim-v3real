...
export function SchoolSelectionModal({
  ...
  const [interviewSchool, setInterviewSchool] = useState<SchoolDef | null>(null);
      ...
      {open && stage && (
        ...
        >
          ...
          >
            {interviewSchool ? (
              <InterviewPanel
                school={interviewSchool}
                onDone={(ratio) => {
                  const id = interviewSchool.id;
                  setInterviewSchool(null);
                  onApply(id, ratio);
                }}
              />
            ) : offer && offerSchool ? (
                ...
                <div className="no-scrollbar flex-1 space-y-2.5 overflow-y-auto p-5">
                  {schoolOptions(character, stage).map((s) => (
                    <SchoolCard
                      ...
                      school={s}
                      onApply={onApply}
                      onInterview={setInterviewSchool}
                    />
                  ))}
                </div>
...
function SchoolCard({
  ...
  c: Character;
  school: SchoolDef;
  onApply: (id: string, interviewRatio?: number) => void;
  onInterview: (school: SchoolDef) => void;
}) {
  const est = admissionEstimate(c, school);
  const applied = c.k12Applied?.includes(school.id) ?? false;
  return (
    <div className="rounded-2xl border border-white/10 bg-white/5 p-3.5">
      ...
      <div className="mt-2.5 flex items-center justify-between gap-2">
        ...
        <button
          disabled={applied}
          onClick={() => (school.interview ? onInterview(school) : onApply(school.id))}
    ...
    <>
      ...
      <div className="p-5">
        <p className="text-sm leading-relaxed text-muted-foreground">
          "Dear {school.name} applicant — the Admissions Committee is delighted to offer you a place
          in the incoming class. We were impressed by your record and believe you will thrive here."
        </p>
        {!showPackage ? (
          <button
            onClick={() => setShowPackage(true)}
            className="mt-4 w-full rounded-2xl border border-primary/50 bg-primary/10 px-4 py-3 text-sm font-bold text-primary transition hover:bg-primary/20"
          >
        ...
        <div className="mt-4 flex gap-2">
          ...
          <button
            ...
            title={showPackage ? undefined : "Review the financial package first"}
            className="flex-1 rounded-2xl bg-primary px-4 py-3 text-sm font-bold text-primary-foreground transition hover:brightness-105 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-40"
          >
            Enroll
          </button>
        </div>
...
/** Interactive admissions interview: four questions, your answers move the odds. */
function InterviewPanel({
  ...
  const questions = useMemo(() => buildQuiz("k12_interview", 4), []);
  ...
  function answer(i: number) {
    ...
    setPicked(i);
    const next = correct + (i === questions[idx].answer ? 1 : 0);
    setCorrect(next);
    setTimeout(() => {
      setPicked(null);
      if (idx + 1 < questions.length) setIdx(idx + 1);
      else onDone(next / questions.length);
    }, 450);
  }
  ...
  return (
    <div className="p-5">
      ...
      <p className="mt-1 text-xs text-muted-foreground">
        The panel is watching how you think. Question {idx + 1} of {questions.length}.
      </p>
      <p className="mt-4 text-sm font-medium leading-relaxed">{questions[idx].q}</p>
