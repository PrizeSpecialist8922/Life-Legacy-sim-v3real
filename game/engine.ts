...
import {
  MAX_ACTIVITIES,
  PREP_INFO,
  buildCompetitionTickets,
  resolveCompetition,
} from "./activities";
import type { PrepLevel } from "./activities";
import {
  CHILD_ACTIONS,
  applyChildAction,
  askOutChance,
...
import type { ChildAction } from "./dating";
...
function maybeEliteRecruiting(c: Character, log: LogEntry[]) {
  ...
  const eligible = JOBS.filter(
    (j) =>
      ...
      (!j.minFitness || c.fitness >= j.minFitness),
  );
...
function applyJobIncome(c: Character, log: LogEntry[]) {
  ...
  // performance drift (burnout drags you down hard)
  let drift = Math.round((c.stats.smarts - 40) / 12 + (Math.random() * 6 - 2));
  if ((c.burnout ?? 0) > 55) drift -= Math.round(((c.burnout ?? 0) - 55) / 12);
  c.job.performance = clamp(c.job.performance + drift);
  if ((c.burnout ?? 0) > 75) c.stats.health = clamp(c.stats.health - 3);
  ...
  for (const r of c.relationships) {
    ...
    if (r.age > 70 && Math.random() < (r.age - 70) / 60) {
      ...
      if (r.type === "mother" || r.type === "father") {
        // Inheritance scales with how well you kept the relationship.
        const share = Math.round(c.family.netWorth * 0.5 * (0.3 + (r.relationship / 100) * 0.7));
        const inherit = Math.max(0, share);
        c.money += inherit;
        c.family.netWorth = Math.max(0, c.family.netWorth - inherit);
        c.stats.happiness = clamp(c.stats.happiness - 12);
        c.log.push({
          age: c.age,
          text: `Your ${r.type} ${r.name} passed away at ${r.age}.${inherit > 0 ? ` They left you ${money(inherit)}.` : ""}`,
          tone: "bad",
        });
      } else {
        c.log.push({ age: c.age, text: `${r.name} passed away at ${r.age}.`, tone: "bad" });
      }
    }
  }
  // Burnout cools off slowly on its own.
  if (c.burnout) c.burnout = clamp(c.burnout - 3);
}
...
export function ageUp(input: Character): AgeUpResult {
  ...
  // Relationships and family grow (or fray) each year.
  partnerTick(c, c.log);
  childrenTick(c, c.log);

  // Each club/sport stages one major competition this academic year.
  c.pendingCompetitions = buildCompetitionTickets(c);
 ...
 *  One application per school per window; interviews (when required) are
 *  interactive and their score shifts the odds. */
export function applyK12School(
  ...
  if (c.k12Applied?.includes(schoolId)) {
    return {
      character: input,
      message: "You've already applied there — one application per school.",
      tone: "bad",
      ok: false,
    };
  }
  ...
  const est = admissionEstimate(c, school, interviewRatio);
  if (Math.random() > est.probability) {
    const msg = `${school.name} declined your application (entrance exam ${est.entranceExamScore}/100, ~${Math.round(est.probability * 100)}% odds). You can apply elsewhere or stay at ${c.edu.school}.`;
    c.log.push({
      age: c.age,
...
export function applyInternship(input: Character, defId: string): ActionResult {
  ...
  if (alreadyAppliedThisYear(c, def.id)) {
    return {
      character: input,
      message: "One application per internship per year — openings refresh when you age up.",
      tone: "bad",
      ok: false,
    };
  }
  c.internApps = { ...(c.internApps ?? {}), [def.id]: c.age };
  if (!spendAction(c)) {
    return {
      character: input,
      message: "No energy left this year. Age up first.",
  ...
  const fullTime =
    (def.level === "college" || def.level === "law") &&
    performance > 0.75 &&
...
// ---------- Club & sport competitions ----------

/** Resolve this year's competition for one activity (free — it IS the event). */
export function completeCompetition(
  ...
  const c: Character = structuredClone(input);
  const ticket = c.pendingCompetitions?.find((t) => t.activity === activity);
  if (!ticket) {
    return {
      character: input,
      message: "No competition pending for that activity.",
      tone: "bad",
      ok: false,
    };
  }
  const outcome = resolveCompetition(c, ticket, prep, quizRatio, c.log);
  c.pendingCompetitions = (c.pendingCompetitions ?? []).filter((t) => t.activity !== activity);
  return { character: c, message: outcome.text, tone: outcome.tone, ok: true };
}

export function skipCompetition(input: Character, activity: string): ActionResult {
  const c: Character = structuredClone(input);
  c.pendingCompetitions = (c.pendingCompetitions ?? []).filter((t) => t.activity !== activity);
  return {
    character: c,
    message: `You sat out this year's ${activity} competition. The team noticed.`,
    tone: "neutral",
    ok: true,
  };
}
...
export function askOut(input: Character, candidate: PartnerState): ActionResult {
  const c: Character = structuredClone(input);
  if (c.age < 16)
    return { character: input, message: "A little young for dating apps.", tone: "bad", ok: false };
  if (c.partner)
    return {
      character: input,
      message: `You're already with ${c.partner.name}.`,
      tone: "bad",
      ok: false,
    };
  if (!spendAction(c)) {
  ...
  c.money -= 100; // dates aren't free
  if (Math.random() > askOutChance(c, candidate)) {
    c.stats.happiness = clamp(c.stats.happiness - 2);
    return {
      character: c,
      message: `${candidate.name} let you down gently. Their loss.`,
      tone: "bad",
      ok: true,
    };
  }
  c.partner = { ...candidate, bond: randInt(30, 45), stage: "dating", yearsTogether: 0 };
  c.stats.happiness = clamp(c.stats.happiness + 6);
  const msg = `${candidate.name} said yes! You're dating a ${candidate.age}-year-old ${candidate.occupation.toLowerCase()} from ${candidate.city}.`;
  c.log.push({ age: c.age, text: msg, tone: "good" });
  return { character: c, message: msg, tone: "good", ok: true };
}

export function spendTimeWithPartner(input: Character): ActionResult {
  const c: Character = structuredClone(input);
  if (!c.partner) return { character: input, message: "You're single.", tone: "bad", ok: false };
  if (!spendAction(c)) {
    return {
      character: input,
      message: "No energy left this year. Age up first.",
  ...
  c.money -= 200;
  c.partner.bond = clamp(c.partner.bond + randInt(8, 14));
  c.stats.happiness = clamp(c.stats.happiness + 4);
  return {
    character: c,
    message: `Quality time with ${c.partner.name}. The bond is real (${c.partner.bond}/100).`,
    tone: "good",
    ok: true,
  };
}

export function progressRelationship(input: Character): ActionResult {
  const c: Character = structuredClone(input);
  const p = c.partner;
  if (!p) return { character: input, message: "You're single.", tone: "bad", ok: false };
  if (!spendAction(c)) {
    return {
      character: input,
      message: "No energy left this year. Age up first.",
  ...
  if (p.stage === "dating") {
    if (p.bond < 45 || p.yearsTogether < 1) {
      return {
        character: input,
        message: "Too soon — build the bond for at least a year first.",
        tone: "bad",
        ok: false,
      };
    }
    p.stage = "exclusive";
    p.bond = clamp(p.bond + 8);
    const msg = `You and ${p.name} are officially exclusive.`;
    c.log.push({ age: c.age, text: msg, tone: "good" });
    return { character: c, message: msg, tone: "good", ok: true };
  }
  if (p.stage === "exclusive") {
    if (p.bond < 60)
      return {
        character: input,
        message: `${p.name} isn't ready — the bond needs to be stronger (60+).`,
        tone: "bad",
        ok: false,
      };
    if (c.money < 4000)
      return {
        character: input,
        message: "Rings cost about $4,000. Save up first.",
        tone: "bad",
        ok: false,
      };
    c.money -= 4000;
    if (Math.random() < 0.15 && p.bond < 75) {
      c.stats.happiness = clamp(c.stats.happiness - 10);
      const msg = `${p.name} said "not yet." The ring goes back in the drawer.`;
      c.log.push({ age: c.age, text: msg, tone: "bad" });
      return { character: c, message: msg, tone: "bad", ok: true };
    }
    p.stage = "engaged";
    p.bond = clamp(p.bond + 10);
    const msg = `${p.name} said YES! You're engaged.`;
    c.log.push({ age: c.age, text: msg, tone: "milestone" });
    return { character: c, message: msg, tone: "milestone", ok: true };
  }
  if (p.stage === "engaged") {
    if (c.money < 15000)
      return {
        character: input,
        message: "The wedding will run about $15,000. Save up.",
        tone: "bad",
        ok: false,
      };
    c.money -= 15000;
    ...
    c.stats.happiness = clamp(c.stats.happiness + 12);
    const msg = `You married ${p.name}! Household finances are now shared.`;
    c.log.push({ age: c.age, text: msg, tone: "milestone" });
    return { character: c, message: msg, tone: "milestone", ok: true };
  }
  return { character: input, message: "You're already married.", tone: "neutral", ok: false };
}

export function breakUp(input: Character): ActionResult {
  const c: Character = structuredClone(input);
  const p = c.partner;
  if (!p) return { character: input, message: "You're single.", tone: "bad", ok: false };
  const divorce = p.stage === "married";
  if (divorce) c.money = Math.round(c.money / 2);
  c.partner = undefined;
  c.stats.happiness = clamp(c.stats.happiness - (divorce ? 12 : 6));
  const msg = divorce
    ? `You divorced ${p.name}. Half of everything walked out the door with them.`
    : `You broke up with ${p.name}.`;
  c.log.push({ age: c.age, text: msg, tone: "bad" });
  return { character: c, message: msg, tone: "bad", ok: true };
}

export function haveBaby(input: Character): ActionResult {
  const c: Character = structuredClone(input);
  const p = c.partner;
  if (!p || p.stage === "dating") {
    return {
      character: input,
      message: "You need a stable relationship (exclusive or beyond) first.",
      tone: "bad",
      ok: false,
    };
  }
  if (p.bond < 50)
    return {
      character: input,
      message: `${p.name} doesn't feel the relationship is ready.`,
      tone: "bad",
      ok: false,
    };
  if ((c.children?.length ?? 0) >= 5)
    return { character: input, message: "Five kids is plenty.", tone: "bad", ok: false };
  if (!spendAction(c)) {
    return {
      character: input,
      message: "No energy left this year. Age up first.",
  ...
  const kid = makeBaby(c);
  c.children = [...(c.children ?? []), kid];
  p.bond = clamp(p.bond + 10);
  c.stats.happiness = clamp(c.stats.happiness + 10);
  const msg = `${p.name} and you welcomed a baby ${kid.gender === "male" ? "boy" : "girl"}: ${kid.name}!`;
  c.log.push({ age: c.age, text: msg, tone: "milestone" });
  return { character: c, message: msg, tone: "milestone", ok: true };
}

export function interactChild(input: Character, kidId: string, action: ChildAction): ActionResult {
  const c: Character = structuredClone(input);
  const kid = c.children?.find((k) => k.id === kidId);
  if (!kid) return { character: input, message: "Child not found.", tone: "bad", ok: false };
  const cost = CHILD_ACTIONS[action].cost;
  if (cost > 0 && c.money < cost) {
    return {
      character: input,
      message: `You need ${money(cost)} for that.`,
      tone: "bad",
      ok: false,
    };
  }
  ...
  const msg = applyChildAction(c, kid, action);
  return { character: c, message: msg, tone: "good", ok: true };
}
...
export function interactParent(
  input: Character,
  relId: string,
  action: ParentAction,
): ActionResult {
  const c: Character = structuredClone(input);
  const r = c.relationships.find((x) => x.id === relId);
  if (!r || !r.alive)
    return { character: input, message: "They're not around anymore.", tone: "bad", ok: false };
  if (!spendAction(c)) {
    return {
      character: input,
      message: "No energy left this year. Age up first.",
  ...
  let msg = "";
  let tone: ActionResult["tone"] = "good";
  switch (action) {
    case "time":
      r.relationship = clamp(rel + randInt(6, 10));
      c.stats.happiness = clamp(c.stats.happiness + 4);
      msg = `You spent real time with ${r.name}. (${r.relationship}/100)`;
      break;
    case "advice":
      r.relationship = clamp(rel + 4);
      c.stats.smarts = clamp(c.stats.smarts + 1);
      msg = `${r.name} shared hard-won advice. You actually listened.`;
      break;
    case "money": {
      const willing = rel >= 55 && c.family.savings > 5000;
      if (!willing) {
        tone = "bad";
        msg =
          rel < 55
            ? `${r.name} said no — your relationship isn't in that place.`
            : `${r.name} would help, but the family savings are thin.`;
        break;
      }
      const amount = Math.min(Math.round(c.family.savings * 0.1), 10000);
      c.family.savings -= amount;
      c.money += amount;
      r.relationship = clamp(rel - 3);
      msg = `${r.name} slipped you ${money(amount)}. "Don't tell your ${r.type === "mother" ? "father" : "mother"}."`;
      break;
    }
    case "tuition": {
      const loan = c.studentLoans.find((l) => l.balance > 0);
      const willing = rel >= 65 && c.family.savings > 20000 && !!loan;
      if (!willing) {
        tone = "bad";
        msg = !loan
          ? "You have no student debt for them to help with."
          : rel < 65
            ? `${r.name} isn't willing — mend the relationship first.`
            : "The family can't spare that kind of money.";
        break;
      }
      const amount = Math.min(Math.round(c.family.savings * 0.25), loan!.balance, 30000);
      c.family.savings -= amount;
      loan!.balance -= amount;
      msg = `${r.name} paid ${money(amount)} toward your student loans. That's love.`;
      break;
    }
    case "medical": {
      const willing = rel >= 60 && c.family.savings > 8000;
      if (!willing || c.stats.health > 70) {
        tone = "bad";
        msg =
          c.stats.health > 70
            ? "You're healthy — no need."
            : `${r.name} can't cover treatment right now.`;
        break;
      }
      c.family.savings -= 8000;
      c.stats.health = clamp(c.stats.health + 15);
      msg = `${r.name} paid for proper treatment. You feel much better.`;
      break;
    }
    case "dinner":
      for (const fam of c.relationships) {
        if (
          fam.alive &&
          (fam.type === "mother" || fam.type === "father" || fam.type === "sibling")
        ) {
          fam.relationship = clamp(fam.relationship + 5);
        }
      }
      c.stats.happiness = clamp(c.stats.happiness + 5);
      msg = "Family dinner — the whole table, no phones. Everyone left warmer.";
      break;
    case "vacation": {
      if (c.family.savings < 6000) {
        tone = "bad";
        msg = "The family can't afford a vacation right now.";
        break;
      }
      c.family.savings -= 6000;
      for (const fam of c.relationships) {
        if (
          fam.alive &&
          (fam.type === "mother" || fam.type === "father" || fam.type === "sibling")
        ) {
          fam.relationship = clamp(fam.relationship + 8);
        }
      }
      c.stats.happiness = clamp(c.stats.happiness + 8);
      msg = "A proper family vacation. The photos will embarrass everyone for decades.";
      break;
    }
    case "gift": {
      if (c.money < 500) {
        tone = "bad";
        msg = "A meaningful gift runs about $500.";
        break;
      }
      c.money -= 500;
      r.relationship = clamp(rel + 8);
      msg = `You gave ${r.name} a thoughtful gift. They teared up a little.`;
      break;
    }
    case "apologize":
      r.relationship = clamp(rel + (rel < 50 ? 12 : 5));
      msg =
        rel < 50
          ? `The apology ${r.name} had been waiting for. Walls came down.`
          : `${r.name} appreciated it, even if things were already fine.`;
      break;
    case "argue":
      r.relationship = clamp(rel - randInt(8, 15));
      c.stats.happiness = clamp(c.stats.happiness - 3);
      tone = "bad";
      msg = `You and ${r.name} had it out. Some things can't be unsaid. (${r.relationship}/100)`;
      break;
  }
  c.log.push({ age: c.age, text: msg, tone: tone === "bad" ? "bad" : "neutral" });
...
export function joinExtracurricular(
  ...
  if (total >= MAX_ACTIVITIES) {
    return {
      character: input,
      message: `Three activities is the limit — commitment beats collecting. Quit one before joining ${name}.`,
      tone: "bad",
      ok: false,
    };
  }
