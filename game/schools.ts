 ...
 *  When an interactive interview was completed, its score replaces the
 *  passive parent-interview proxy. */
export function admissionEstimate(
  ...
  const interviewScore =
    interviewRatio !== undefined
      ? interviewRatio * 100
      : parentRelationshipAvg(c) * 0.5 + c.stats.happiness * 0.3 + 20;
  ...
  if (school.interview)
    p += ((interviewScore - 55) / 100) * (interviewRatio !== undefined ? 0.35 : 0.2);
  if (!character) p -= 0.25;
