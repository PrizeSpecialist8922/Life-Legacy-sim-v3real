...
export type InternshipLevel = "high" | "college" | "law";
...
export const INTERNSHIPS: InternshipDef[] = [
  ...
  // Law school only — Summer Associate roles are closed to undergraduates.
  {
    id: "law-sa",
    ...
    prestige: 93,
    minGpa: 3.3,
    blurb: "THE Big Law pipeline. Strong summers convert to associate offers.",
  },
  {
    id: "law-judicial",
    name: "Judicial Internship",
    org: "Federal District Court",
    field: "Law",
    level: "law",
    prestige: 85,
    minGpa: 3.4,
    blurb: "Clerk for a judge; unbeatable for recommendation letters.",
  },
  {
    id: "law-prosecutor",
    name: "Prosecutor Internship",
    org: "District Attorney's Office",
    field: "Law",
    level: "law",
    prestige: 72,
    minGpa: 3.0,
    blurb: "Real courtroom exposure on the state's side.",
  },
  {
    id: "law-defender",
    name: "Public Defender Internship",
    org: "Public Defender's Office",
    field: "Law",
    level: "law",
    prestige: 68,
    minGpa: 2.8,
    blurb: "Defense work with immediate client contact.",
  },
];
...
export function internshipEligibility(c: Character): {
  ...
  // Law students recruit from a dedicated pool (Summer Associate etc.).
  if (
    c.education === "gradschool" &&
    (c.gradProgram?.kind === "jd" || c.gradProgram?.kind === "jdmba")
  ) {
    return { eligible: true, level: "law" };
  }
...
/**
 * Openings refresh every academic year: a rotating subset of each level's
 * pool is hiring, seeded by the year so the list is stable until you age up.
 */
export function availableInternships(c: Character, level: InternshipLevel): InternshipDef[] {
  const pool = INTERNSHIPS.filter((d) => d.level === level);
  if (pool.length <= 4) return pool;
  const seed = c.age * 7 + c.birthday.length + c.name.length;
  return pool.filter((_, i) => (i + seed) % 3 !== 0);
}

/** One application per internship per year. */
export function alreadyAppliedThisYear(c: Character, defId: string): boolean {
  return c.internApps?.[defId] === c.age;
}
